...
public:
	libvpx_vp9_ram_decoder(stream_desc* upstream, stream_desc* downstream, uint32_t threads = 1, int flags = VPX_CODEC_USE_POSTPROC):
		...
		if (downstream) {
			downstream->type = stream_desc::MTYPE_VIDEO;
			downstream->detail.video.codec = stream_desc::video_info::VCODEC_RAW;
			downstream->detail.video.width = upstream->detail.video.width;
			downstream->detail.video.height = upstream->detail.video.height;
	...
	virtual int Dropped() override final
	{
...
video_decoder* video_decoder_factory::CreateDefaultVP9Decoder(stream_desc* upstream, uint32_t threads, bool use_post_proc)
{
	...
	if(threads>1)
		...
		threads = 1;
	return new libvpx_vp9_ram_decoder(upstream,nullptr,threads,flags);
}
